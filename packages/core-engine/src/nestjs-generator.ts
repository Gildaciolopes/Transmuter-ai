 ...
 *
 * @param dtoPathMap - optional map of DTO class name → relative import path (no .ts extension)
 *   e.g. Map { "RequestProduct" => "../domain/product/request-product" }
 *   When absent, falls back to same-directory `./${kebab}.dto` import.
 */
export function generateNestController(
  cls: ParseResult,
  dtoPathMap?: Map<string, string>,
): string {
  const className = cls.className;
  ...
  lines.push(
    `import { ${[...decoratorImports].sort().join(", ")} } from '@nestjs/common';`,
  );
  lines.push(`import { ${serviceName} } from './${resource}.service';`);
  for (const dto of dtoImports) {
    const importPath = dtoPathMap?.get(dto) ?? `./${toKebabCase(dto)}.dto`;
    lines.push(`import { ${dto} } from '${importPath}';`);
  }
  lines.push("");
  lines.push(`@Controller('${resource}')`);
  lines.push(`export class ${className} {`);
  lines.push(
    `  constructor(private readonly ${lcFirst(serviceName)}: ${serviceName}) {}`,
  );
  lines.push("");
...
function renderMethod(method: MethodInfo): string[] {
  ...
  const nestPath = method.path ? method.path.replace(/\{(\w+)\}/g, ":$1") : "";
  ...
  for (const p of method.params) {
    if (p.source === "path")
      params.push(`@Param('${p.name}') ${p.name}: string`);
    else if (p.source === "query")
      params.push(`@Query('${p.name}') ${p.name}: string`);
    else if (p.source === "header")
      params.push(`@Headers('${p.name}') ${p.name}: string`);
  }
  if (method.bodyType) {
    const tsType =
      isPrimitive(method.bodyType) || method.bodyType === "Object"
        ? "unknown"
        : method.bodyType;
