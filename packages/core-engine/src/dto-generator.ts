...
export function generateDto(dto: ParseResult): string {
  ...
  for (const field of dto.fields) {
    if (field.isTransient) continue;
    let zodType = javaToZod[field.type] ?? 'z.unknown()';
  ...
  lines.push('});');
  lines.push('');
  lines.push(`export type ${dto.className}Validated = z.infer<typeof ${dto.className}Schema>;`);

  return lines.join('\n');
}

function applyConstraints(zodBase: string, constraints: ConstraintInfo): string {
  let chain = zodBase;
